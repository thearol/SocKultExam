...
library(pacman)
p_load(lme4, lmerTest, brms, tidyverse, car)
...
get_prior(Surrender ~ skill_dif_answer * Leader_gender * Follower_gender + (1 + skill_dif_answer:Follower_gender | Subject ), data = no_pool_disagree, family = "bernoulli")
...
```{r}
conditions <- make_conditions(m_no_pool_correct, "skill_dif_correct")

wuu <- marginal_effects(m_no_pool_correct, "Leader_gender:Follower_gender", conditions = conditions, select_points = 0.1)
...
prediction_mat <- data.frame(skill_dif_correct = no_pool_disagree$skill_dif_correct, 
                         Leader_gender =rep(levels(no_pool_disagree$Leader_gender),each = nrow(no_pool_disagree)),
                         Follower_gender =rep(levels(no_pool_disagree$Follower_gender),each = nrow(no_pool_disagree)))
...
test$Surrender <- predict(m_no_pool_correct, newdata = test, re_formula = NULL)

# nd <- tibble(skill_dif_correct= seq(from = -1, to = 1, length.out = 2000) %>% 
#            rep(., times = 2),
#          Leader_gender = rep(0:1, each = 2000),
#          Follower_gender = rep(0:1, each = 2000),
#          Subject = NA)

predicted <- predict(m_no_pool_correct, newdata = nd)


# rearranging the fitted values and the values used for predictions
df <- data.frame(predicted,
             skill_dif_correct = no_pool_disagree$skill_dif_correct,
             Leader_gender =rep(levels(no_pool_disagree$Leader_gender),each = nrow(no_pool_disagree)),
             Follower_gender =rep(levels(no_pool_disagree$Follower_gender),each = nrow(no_pool_disagree)))

# plotting the fitted regression lines
ggplot(test, aes(x = skill_dif_correct, y = Surrender, group = Leader_gender, col = Leader_gender)) + 
geom_line() + 
