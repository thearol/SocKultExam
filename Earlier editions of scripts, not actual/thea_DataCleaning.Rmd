...
for (i in unique(left_data$unique_ID_left)){
  ...
  if (n == 1){
    ...
    Results_left <- rbind(Results_left, data_i)
  }
...
```{r}
# 
data_i <- filter(data, unique_ID_right == "11_Matilde")
...
#  prior(normal(0,2), class = "b"), 
  prior(normal(0,125), class = "b", coef = "dif_blue")
  )
...
pp_check(prior_check, nsamples = 1000)


summary(m)
jpeg(file=pp_left_path)
plot(marginal_effects(m), points = TRUE, jitter = TRUE)
dev.off()
...
for (i in unique(right_data$unique_ID_right)){
  ...
  #Insert into permanent data frame
  if (n == 1){
    Results_right <- data_i
  } else {
    Results_right <- rbind(Results_right, data_i)
  }
...
for (i in unique(joint_data$GroupNumber)){
  ...
  #Insert into permanent data frame
  if (n == 1){
    Results_joint <- data_i
  } else {
    Results_joint <- rbind(Results_joint, data_i)
  }
...
- Gem rhat, efficient samples *
- plot(m) - transitions *
- filter(correct)*
- pp_check udenfor loop for 5 random participants
