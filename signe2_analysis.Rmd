...
nd <- 
  tibble(skill_dif_correct= c(min(pool_disagree$skill_dif_correct),1,max(pool_disagree$skill_dif_correct)) %>% 
           rep(., times = 1000),
         Follower_gender=factor(0:1) %>% rep(., times = 1500),
         Leader_gender = factor(0:1) %>% rep(., times = 1500),
         Subject = NA)
...
#skal vi ikke rescale skill_diff? så vil mean være lig 0?
