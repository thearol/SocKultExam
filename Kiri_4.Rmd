...
prior_answer =  c(
  ...
  prior(normal(0.5,0.17), class = "Intercept"),
  prior(normal(0,0.125), class = "sd", coef = "dif_blue", group = "unique"),
  prior(normal(0.5,0.17), class = "sd", coef = "Intercept", group = "unique")
)
...
prior_check <- brm( answer ~ dif_blue + (dif_blue|unique), prior = prior_answer,
           data = ldata, sample_prior = "only",iter = 4000, family = "bernoulli")

#Prior predictive check - for correct
prior_check_c <- brm(Correct ~  0 + dif_blue_abs + (0 +dif_blue_abs|unique), prior = prior_correct,
           data = ldata, sample_prior = "only",iter = 4000, family = "bernoulli")
...
#Saving the estimates
  ...
  betas$m.prior.1.81..3. <- str_remove(betas$m.prior.1.81..3., "unique")
  #Insert betas in dataframe
  betas$Est_individual_p_answer <- fixef(m_p)[1:81] + ranef(m_p)[1:81]
  betas$Est_idividual_p_correct <- fixef(m_p_c)[1:81] + ranef(m_p)[1:81]
  
#Merging with betas with results
  ...
  #Renaming column names to the left
...
prior_answer =  c(
  ...
  prior(normal(0.5,0.17), class = "sd", coef = "Intercept", group = "GroupNumber")
)
...
#Prior predictive check - for answer
prior_check <- brm(joint_answer ~ dif_blue + (dif_blue|GroupNumber), prior = prior_answer_joint,
           data = ldata, sample_prior = "only",iter = 4000, family = "bernoulli")

#Prior predictive check - for correct
prior_check_c <- brm(Correct_joint ~  0 + dif_blue_abs + (0 +dif_blue_abs|GroupNumber), prior = prior_correct_joint,
           data = ldata, sample_prior = "only",iter = 4000, family = "bernoulli")
...
###CHECK OM DET HER VIRKER
#Saving the estimates
  #Saving summary in temporary dataframe
  betas <- data.frame(m_joint_answer$prior[1:?, 3])
  #Removing unique from the string
  betas$m.prior.1.81..3. <- str_remove(betas$prior_check.prior.1.?..3., "unique")
  #Insert betas in dataframe
  betas$Est_individual_p_answer <- fixef(m_p)[1:?] + ranef(m_p)[1:81]
  betas$Est_idividual_p_correct <- fixef(m_p_c)[1:81] + ranef(m_p)[1:81]
