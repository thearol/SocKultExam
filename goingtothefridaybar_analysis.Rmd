...
no_pool_estimates$Surrender[no_pool_estimates$chosen_leader == "Agree"] <- NA
...
get_prior(Surrender ~ skill_dif_correct * Leader_gender * Follower_gender + (1 + skill_dif_correct:Follower_gender | Subject ), data = pool_long, family = "bernoulli")
...
```{r}
nd <- 
  tibble(skill_dif_correct= seq(from = -63, to = 6.3, length.out = 30) %>% 
           rep(., times = 2),
         Leader_gender = rep(0:1, each = 30),
         Follower_gender = rep(0:1, each = 30))
